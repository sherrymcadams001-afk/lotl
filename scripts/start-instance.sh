...
CHROME_PORT="${2:-9222}"
HOST="${HOST:-127.0.0.1}"
USER_DATA_DIR="${USER_DATA_DIR:-/tmp/chrome-lotl-${CHROME_PORT}}"
...
nohup node "$ROOT_DIR/scripts/start-controller.js" --host "$HOST" --port "$CONTROLLER_PORT" --chrome-port "$CHROME_PORT" \
